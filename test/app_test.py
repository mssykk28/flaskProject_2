...
def test_get_all_requests():
    ...
    response = requests.get("%s/book" % (BASE_URL))
    assert_true(response.ok)


def test_get_individual_request():
    """Test getting an individual book"""
    response = requests.get(
        "%s/book/04cfc704-acb2-40af-a8d3-4611fab54ada" % (BASE_URL))
    assert_true(response.ok)


def test_get_individual_request_404():
    """Test getting a non existent book"""
    response = requests.get("%s/book/an_incorrect_id" % (BASE_URL))
...
def test_add_new_record():
    ...
    response = requests.post("%s/book" % (BASE_URL), json=payload)
...
def test_get_new_record():
    ...
    url = "%s/book/%s" % (BASE_URL, NewUUID.value)
...
def test_edit_new_record_title():
    ...
    response = requests.put("%s/book/%s" %
...
def test_delete_new_record():
    ...
    response = requests.delete("%s/book/%s" % (BASE_URL, NewUUID.value))
